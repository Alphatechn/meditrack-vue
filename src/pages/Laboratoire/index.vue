              ...
              >
                <thead>
                  <tr>
                    ...
                    <th scope="col">Date examination</th>
                    <th scope="col">Action</th>
                  </tr>
                </thead>
                <tbody>
                  <tr
                    v-for="(examination, index) in allPatCons"
                    :key="examination.id_consultation"
                  >
                    <td>{{ compter + index }}</td>
                    <td>{{ examination.code }}</td>
                    <td>{{ examination.nom }} {{ examination.prenom }}</td>
                    <td>{{ examination.date_recu}}</td>

                    <td>
                      <Options :examination="examination" />
                    </td>
                  </tr>
                </tbody>
              </table>
...
export default {
  ...
  methods: {
    ...
    async fetchConsultation() {
      const examination = await fetch(
        "http://127.0.0.1:8000/api/examination/user/" +
      ...
      const Examination = await examination.json();
      return Examination;
    },
